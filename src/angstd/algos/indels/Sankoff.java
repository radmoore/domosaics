 ...
 *
 */
public class Sankoff extends AbstractReconstructionAlgo {
	...
	private enum State {
		...
		/**
		 * Whether or not the specified domain is present within the actual
		 * column of the domain column vector
		 * 
		 * @param da
...
//		}
	...
	public Sankoff(DomainTreeViewI view, boolean useSets) {
		super(view, useSets);
	}
    ...
    protected String doInBackground() {
		try {		
			// calculate the aligned domain vector
...
//				sankoff_Down(lca, costs, domCols.get(actCol));
				sankoff_Up(tree.getRoot(), tree.getRoot(), costs, domCols.get(actCol));	
...
//				if (state.observed(da, dom))
	...
	private void sankoff_Down(TreeNodeI root, CostMatrix costs, Domain dom) {
		...
		if (ancestralStates.contains(State.PRESENT)) {
			...
			if (!root.isLeaf())
				node2da.get(root).add(new GapDomain(null));
	...
	private void sankoff_Down(State state, TreeNodeI actNode, CostMatrix costs, Domain dom) {
		...
		if (ancestralStates.contains(State.PRESENT)) {
			...
			if (!actNode.isLeaf())
				node2da.get(actNode).add(new GapDomain(null));
...
// old calculation method containing test code
//@Override
//protected String doInBackground() {
//	try {
//	// BEGIN ONLY TESTCODE: DISPLAYS INNER ARRANGEMENTS
////	if (testDAs != null) {
////		DomainTreeViewI view = ViewHandler.getInstance().getActiveView();
////		for (DomainArrangement da : testDAs) {
////			da.getTreeNode().setArrangement(null);
////			for (Domain dom : da.getDomains())
////				view.getDomainComponentManager().removeComponent(dom);
////			view.getArrangementComponentManager().removeComponent(da);
////		}
////		view.getDomainTreeLayoutManager().structuralChange();
////	}
////	testDAs = new ArrayList<DomainArrangement>();
//	// END ONLY TESTCODE: DISPLAYS INNER ARRANGEMENTS
//	
//	// do some initialization
//	node2da = new HashMap<TreeNodeI, DomainSet>();
//	node2scoreVec = new HashMap<TreeNodeI, Double[]>();
...
//		node2da.put(node, new DomainSet(node.getArrangement()));
//		
...
//		int percent = (int) Math.round(100 / (double) doms.size() * (i+1));
////		listener.updateProgress("Processing "+doms.get(i).getFamID()+" ("+(i+1)+"/"+doms.size()+")", percent);
//		listener.updateProgress(percent+"%", percent);
//		
////		if (!doms.get(i).getID().equals("Toxin_24"))
////			continue;
//		// get Hotspot this is the maximum node where the domain can be inserted
////		lca = HotspotAlgo.findHotspot(doms.get(i), tree);
////		sankoff_Up(lca, tree, costs, doms.get(i));	
////		sankoff_Down(lca, costs, doms.get(i));
//		
//		sankoff_Up(tree.getRoot(), tree, costs, doms.get(i));		
//		sankoff_Down(tree.getRoot(), costs, doms.get(i));
//	}
//	
//	
//	
//	// BEGIN ONLY TESTCODE: DISPLAYS INNER ARRANGEMENTS
////	for (TreeNodeI actNode : node2da.keySet()) {
////		if (actNode.isLeaf())
////			continue;
////		DomainArrangement da = new DomainArrangement();
////		for (int x = 0; x < node2da.get(actNode).size(); x++)
////			da.addDomain(new Domain(1, 100, node2da.get(actNode).get(x).getFamily()));
////		actNode.setArrangement(da);	
////		testDAs.add(da);
////	}
//	// END ONLY TESTCODE: DISPLAYS INNER ARRANGEMENTS
//	}catch(Exception e) {
