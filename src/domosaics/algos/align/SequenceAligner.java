 ...
 * -----------------<br>
 * Align                  general pairwise alignment<br>
 *   AlignSimple          alignment with simple gap costs<br>
 *      NW                global alignment with simple gap costs<br>
 *      SW                local alignment with simple gap costs<br>
 *      RM                repeated matches with simple gap costs<br>
 *      OM                overlap matches with simple gap costs <br>
 *   AlignAffine          alignment with affine gap costs (FSA model)<br>
 *      NWAffine          global alignment with affine gap costs<br>
 *   AlignSmart           alignment using smart linear-space algorithm<br>
 *      NWSmart           global alignment using linear space<br>
 *      SWSmart           local alignment using linear space<br>
 *   AlignSmartAffine     alignment w affine gap costs in linear space<br>
 *     SWSmartAffine     local alignment w affine gap costs in linear space<br>
 *  Traceback             traceback pointers<br>
 *   Traceback2           traceback for simple gap costs<br>
 *   Traceback3           traceback for affine gap costs<br>
 * Substitution            substitution matrices with fast lookup<br>
 *   Blosum50             the BLOSUM50 substitution matrix<br>
 * Output                  general text output<br>
 *   SystemOut            output to the console (in the application)<br>
 *   TextAreaOut          output to a TextArea (in the applet)<br>
 * <p>
...
/**
 * This implementation is based on the "Implementation of some algorithms for pairwise alignment from
 * Durbin et al: Biological Sequence Analysis, CUP 1998, chapter 2.
 * Peter Sestoft, sestoft@itu.dk 1999-09-25, 2003-04-20 version 1.4
 * Reference:  http://www.itu.dk/people/sestoft/bsa.html ".
 * <p>
 * For details look into the source code.
 * <p>
 * 
 */
public class SequenceAligner {
	...
	public int align(SequenceI seq1, SequenceI seq2) {
...
//		 NWAffine algo = new NWAffine(sub, 12, 2, seq1.getSeq(false), seq2.getSeq(false));
		
		 NW algo =  new NW (sub, 8,  seq1.getSeq(false), seq2.getSeq(false));
		 
		 String[] aligned =  algo.getMatch();
		 int count = 0;
		 for (int i = 0; i < aligned[0].length(); i++)
			 if (aligned[0].charAt(i) == aligned[1].charAt(i))
				 if (aligned[0].charAt(i) != '-')
					 count++;

		 double percent =  count / (double)aligned[0].length()  * 100;
...
abstract class Align {
	 ...
	 * 
	 * @param tb
	 * 		actual traceback state
	 * @return
	 * 		next traceback state
	 */
...
abstract class AlignSmartAffine extends Align {
	...
	public AlignSmartAffine(Substitution sub, int d, int e, String sq1, String sq2) {
		super(sub, d, sq1, sq2);
		this.e = e;
		F = new int[3][2][m+1];
	}
